	  ...
	  <div class="acdnPanel">
		<div class="panelInner">
		  <div class="searchBoxFrom">
			<form action="{{ route('tekute-search') }}" method="get" name="topSearch" id="topSearch">
				  ...
				  <dd>
					<ul class="fList">
                      ...
                      @foreach ($place_tags as $tag)
						<li><input id="s{{ $idx }}" name="pt[]" type="checkbox" value="{{ $tag->id }}" @if (isset($selected_tag) && $tag->name == $selected_tag) checked @endif><label for="s{{ $idx }}" class="{{ $tag->class }}">{{ format_tekute_shop($tag->name) }}</label></li>
                        @php $idx++; @endphp
                      @endforeach
					</ul>
				  </dd>
