...
<script src="/assets/js/analytics.js"></script>
	...
	<meta name="description" content="JR東日本東北総合サービス株式会社の最新情報をご覧いただけます。">
	<meta name="keywords" content="">
	<title>ニュースリリース｜JR東日本東北総合サービス株式会社【LiViT】</title>
				...
				<div class="standard-box">
					...
					<main class="articleOuter">
						...
						@foreach($news_release as $news)
							<article class="summaryBox">
								<div class="info">
									<time datetime="{{ format_date_s($news->publication_date_time_start) }}">{{ format_date_s($news->publication_date_time_start) }}</time>
									@if ( $news->tag_names )
										<ul class="tag">
										@foreach (explode(",",$news->tag_names) as $tag)<li>{{ $tag }}</li>@endforeach
										</ul>
									@endif
								</div>
								@switch( $news->link_type ) 
									@case(1)
										<!-- {{-- URL --}} -->
										<p class="summary"><a href="{{ $news->url }}">{!! $news->title !!}</a></p>
										@break
									  
									@case(2)
										<!-- {{-- PDF --}} -->
										<p class="summary"><a href="{{ $news->pdf_link_url }}" target="_blank" >{!! $news->title !!}</a></p>
										@break
									  
									@case(3)
										<!-- {{-- 記事 --}} -->
										<p class="summary"><a href="/company/news-release/article/{{ $news->id }}">{!! $news->title !!}</a></p>
										@break
									  
									@case(4)
									<!-- {{-- リンクなし --}} -->
										<p class="summary">{!! $news->title !!}</p>
										@break
						...
						<ul class="pager">
							{{ $news_release->links('company.common.pagination') }}
						</ul>
